module Doocr
	...
	class_getter cheat_mus : Pointer(Cheatseq) = cheat_mus_storage.to_unsafe
	class_getter cheat_god : Pointer(Cheatseq) = cheat_god_storage.to_unsafe
	class_getter cheat_ammo : Pointer(Cheatseq) = cheat_ammo_storage.to_unsafe
	class_getter cheat_ammonokey : Pointer(Cheatseq) = cheat_ammonokey_storage.to_unsafe
	class_getter cheat_noclip : Pointer(Cheatseq) = cheat_noclip_storage.to_unsafe
	class_getter cheat_commercial_noclip : Pointer(Cheatseq) = cheat_commercial_noclip_storage.to_unsafe
	class_getter cheat_choppers : Pointer(Cheatseq) = cheat_choppers_storage.to_unsafe
	class_getter cheat_clev : Pointer(Cheatseq) = cheat_clev_storage.to_unsafe
	class_getter cheat_mypos : Pointer(Cheatseq) = cheat_mypos_storage.to_unsafe
	class_getter cheat_amap : Pointer(Cheatseq) = cheat_amap_storage.to_unsafe
	class_getter cheat_me : Pointer(Cheatseq) = cheat_me_storage.to_unsafe
	...
	class_getter activeplats : Array(Pointer(CDoom::Plat)) = Array.new(CDoom::MAXPLATS, Pointer(CDoom::Plat).null)
	class_getter activeceilings : Array(Pointer(CDoom::Ceiling)) = Array.new(CDoom::MAXCEILINGS, Pointer(CDoom::Ceiling).null)
	class_getter itemrespawnque : Array(CDoom::Mapthing) = Array.new(CDoom::ITEMQUESIZE) { CDoom::Mapthing.new }
	class_getter blocklinks : Array(Pointer(CDoom::Mobj)) = [] of Pointer(CDoom::Mobj)
	class_getter channels : Array(Pointer(UInt8)) = Array.new(CDoom::NUM_CHANNELS, Pointer(UInt8).null)
	class_getter channels_s_sound : Array(CDoom::Channel) = Array.new(CDoom::NUM_CHANNELS) { CDoom::Channel.new }
	...
	class_getter zlight : Array(Pointer(CDoom::Lighttable)) = Array.new(CDoom::LIGHTLEVELS * CDoom::MAXLIGHTZ, Pointer(CDoom::Lighttable).null)
	...
	class_getter drawsegs : Array(CDoom::Drawseg) = Array.new(CDoom::MAXDRAWSEGS) { CDoom::Drawseg.new }
	class_property ds_p : Pointer(CDoom::Drawseg) = Pointer(CDoom::Drawseg).null
	class_getter vissprites : Array(CDoom::Vissprite) = Array.new(CDoom::MAXVISSPRITES) { CDoom::Vissprite.new }
	class_getter vsprsortedhead : Array(CDoom::Vissprite) = [CDoom::Vissprite.new]
	class_getter intercepts : Array(CDoom::Intercept) = Array.new(CDoom::MAXINTERCEPTS) { CDoom::Intercept.new }
	...
	class_getter localcmds : Array(CDoom::Ticcmd) = Array.new(CDoom::BACKUPTICS) { CDoom::Ticcmd.new }
	class_getter netcmds : Array(StaticArray(CDoom::Ticcmd, CDoom::MAXPLAYERS)) = Array.new(CDoom::BACKUPTICS) { StaticArray(CDoom::Ticcmd, CDoom::MAXPLAYERS).new(CDoom::Ticcmd.new) }
	...
	class_getter events : Array(CDoom::Event) = Array.new(CDoom::MAXEVENTS) { CDoom::Event.new }
	class_getter screens : Array(Pointer(UInt8)) = Array.new(5, Pointer(UInt8).null)
	class_property numcmaps : Int32 = 0
	class_getter deathmatchstarts : Array(CDoom::Mapthing) = Array.new(CDoom::MAX_DM_STARTS) { CDoom::Mapthing.new }
	...
	class_getter scalelightfixed : Array(Pointer(CDoom::Lighttable)) = Array.new(CDoom::MAXLIGHTSCALE, Pointer(CDoom::Lighttable).null)
	class_getter scalelight : Array(Array(Pointer(CDoom::Lighttable))) = Array.new(CDoom::LIGHTLEVELS) { Array.new(CDoom::MAXLIGHTSCALE, Pointer(CDoom::Lighttable).null) }
